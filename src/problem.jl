...
struct QuboProblem{T<:AbstractFloat,Tw<:AbstractMatrix{T},Tc<:Union{AbstractVector{T},Nothing}}
    W::Tw
    has_bias::Bool
    c::Tc
    N::Int
    ...
    function QuboProblem(
        ...
        all(iszero, W[diagind(W)]) || throw(ArgumentError("Matrix W must have zero diagonal"))
        (!has_bias || length(bias) == N) || throw(ArgumentError("Bias vector must have length $N"))
...
get_energy(problem::QuboProblem, configuration::AbstractVector{<:Real}) = get_energy(problem, Int8.(configuration))

add_solution(problem::QuboProblem, sol::Solution) = push!(problem.solutions, sol)

function add_solution(problem::QuboProblem, configuration::Vector{Int8}, solver::AbstractSolver; kwargs...)
...
function add_solution(problem::QuboProblem, configuration::Vector{<:Real}, solver::AbstractSolver; kwargs...)
